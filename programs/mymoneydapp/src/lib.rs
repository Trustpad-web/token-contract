...
declare_id!("EeuYUHgTybmUwweikf2CdGBSFoHNpVNjHVgjUzRPfiLo");
...
pub mod mymoneydapp {
    ...
    pub fn proxy_mint_to(ctx: Context<ProxyMitTo>, amount: u64) -> ProgramResult {
        token::mint_to(ctx.accounts.info(), amount)
    }

    pub fn proxy_burn (ctx: Context<ProxyBurn>, amount: u64) -> ProgramResult {
        token::burn(ctx.accounts.info(), amount)
    }
    ...
    pub fn proxy_set_authority(
        ...
        token::set_authority(ctx.accounts.info(), authority_type.info(), new_authority)
    }
}
